This is a AGAVE template for a UH HPC MAFFT (http://mafft.cbrc.jp/alignment/software/) application. 
...
Create file in your ~/lus directory called input.fasta it can be blank for the test if you just want to be sure MAFFT launches. Be sure to specify a real MAFFT input file for the default value - the current value will be a input.fasta file in you ~/lus directory if you put your MAFFT file there and name it "input.fasta" this app will use it.

 Now navigate to your ~/lus/apps/uh-mafft-7.245/test folder and run the test.sh script to be sure it runs - if should create a input and output folder and copy the prodigal input file to the input folder and write the standard out  to a file in the output folder.  Please do this in an interactive sandbox session vs on the login nodes.
 ...
 >jobs-template -A uh-mafft-7.245 > mysubmit.json
